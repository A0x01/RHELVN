...
function createPostgresConfig() {
  ...
  cat /etc/postgresql/12/main/conf.d/postgresql.custom.conf
   sed -i 's/port = 5433/port = 5432/' /etc/postgresql/12/main/postgresql.conf
...
if [ "$1" = "import" ]; then
    ...
    if [ ! -f /data.osm.pbf ] && [ -z "${DOWNLOAD_PBF:-}" ]; then
        ...
        DOWNLOAD_PBF="https://download.geofabrik.de/asia/iraq-latest.osm.pbf"
        DOWNLOAD_POLY="https://download.geofabrik.de/asia/iraq.poly"
    fi
